 ...
 */
   ...
   * @private
   */
  _orderNestedGroups(groupIds, indexedGroups, sortedClustered) {
	var _this6 = this;
    /**
	...
	function getOrderedNestedGroups(t, groupIds, indexedGroups, sortedClustered) {
		...
		groupIds.forEach(function (groupId) {
			...
			if (groupData.nestedGroups) {
				var nestedGroupIds = [];
				if (sortedClustered.hasOwnProperty(groupId)) {//essentially "Dictionary.containskey(groupId)"
  ...
  _onGroupClick(event) {
	...
	this.body.emitter.emit('nestchanged', { groupId: group.groupId, showNested: group.showNested }); // fire event
  }
