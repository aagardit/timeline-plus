...
class Core {
  ...
  _redraw() {
	...
	if(Math.abs(this.props.storedAutoHeight - autoHeight) > 1) { //This is to fix an issue with an odd 'jitter' when custom scaling is applied
      dom.root.style.height = util.option.asSize(options.height, `${autoHeight}px`);
      this.props.storedAutoHeight = autoHeight;
	}
    else
      dom.root.style.height = this.props.storedAutoHeight;
